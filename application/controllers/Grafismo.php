...
class Grafismo extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->rutaTrazo_model->insertar_rutaTrazo($data4)) {
			$this->load->view('layout/header_grafismo');
			$this->load->view('grafismo/index');
			$this->load->view('layout/footer');
		} else {
			echo "Error en el registro.";
			$this->load->view('layout/grafismo');
			$this->load->view('grafismo/index');
			$this->load->view('layout/footer');
		}
	}
	...
	public function guardarImagen()
	{
		if ($this->input->post('imagen')) {
			...
			if (preg_match('/^data:image\/(png|jpeg|jpg);base64,/', $imagenData)) {
				...
				$ruta_guardado = 'almacenamiento/galeria/galeria-grafismo/';
				$identificador_usuario = $this->session->userdata('identificador');
